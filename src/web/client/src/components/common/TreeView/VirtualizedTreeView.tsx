...
interface VirtualizedTreeViewProps<T extends TreeNode> {
  ...
  height?: number;  // 容器高度
  baseItemHeight?: number;  // 基础项高度
  expandedIds: Set<string>;  // 外部传入的展开状态
  onToggleExpand: (id: string) => void;  // 展开/折叠回调
  useFixedHeight?: boolean;  // 是否使用固定高度模式（默认 false 使用可变高度）
}
